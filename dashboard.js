...
function mostrarConsultasDia(data) {
    ...
    const consultasDia = consultas.filter(c => {
        ...
        return dataConsulta.getTime() === data.getTime();
    });
    ...
    container.innerHTML = `<h3>${data.toLocaleDateString('pt-BR', { day: 'numeric', month: 'long' })}</h3>`;
...
function carregarPagamentos() {
    ...
    pagamentos.forEach((pagamento, index) => {
        ...
        if (pagamento.pago) {
            ...
            item.style.background = 'linear-gradient(135deg, #fee2e2 0%, #fecaca 100%)';
        }
